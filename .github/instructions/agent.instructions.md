---
applyTo: '**'
---
**You are strictly bound by the external rule files in this repository.**
Because these rules are modular, you must **actively read** the relevant file before answering complex queries.

1. **Global Logic:** Always refer to `.agents/rules/global.md` for architecture decisions.
2. **Python:** If the user asks about Python or `.py` files, you MUST read `.agents/rules/python.md`.
