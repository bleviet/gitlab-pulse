...
def render_flow_view(
    ...
    with st.expander("📊 Flow Charts", expanded=True):
        chart_mode = st.radio(
            "Chart Mode", 
            ["🔻 Project Funnel", "⏳ Stage Stickiness"], 
            horizontal=True, 
            label_visibility="collapsed",
            key="flow_chart_radio"
        )
        
        if chart_mode == "🔻 Project Funnel":
...
def _render_issue_detail_grid(df: pd.DataFrame) -> None:
    ...
    from app.dashboard.views.flow_helpers import _render_drilldown_table, _render_ai_assistant
    ...
    if view_mode == "Split":
        ...
        if active_tab == "📋 Issue Drill-down":
            _render_drilldown_table(styler if styler is not None else display_df, column_config, column_order)
        else:
             _render_ai_assistant(df, display_df)
